...
function PositionModal({ target, holders, levels, kinds, open, onClose, onSaved }: {
  ...
  useEffect(() => {
    ...
    if (target?.mode === 'edit' && target.node) {
      ...
      form.setFieldsValue({ title_template: target.template.title_template, access_level_id: target.template.access_level_id, event_kind_id: target.template.event_kind_id });
