"""Event kinds (Competition/Training/R&D/...) and per-kind automatic roles:
every kind reuses the competition structure with its own labels, and a role
...
def _kinds(login) -> dict[str, dict]:
    return {k["slug"]: k for k in login("admin").get("/api/competitions/kinds").json()}
...
def test_kind_crud_is_org_edit_gated(login, org):
    ...
    assert login("cto").post("/api/competitions/kinds", json={
        "name": "Workshop", "event_label": "Workshop"}).status_code == 403
    r = login("ceo").post("/api/competitions/kinds", json={  # ceo=Exec has org.edit
    ...
    # rename a label
    assert login("ceo").patch(f"/api/competitions/kinds/{kind['id']}",
                              json={"team_label": "Squad"}).status_code == 200
    assert _kinds(login)["workshop"]["team_label"] == "Squad"

    # empty kind deletes fine
    assert login("ceo").delete(f"/api/competitions/kinds/{kind['id']}").status_code == 204
...
def test_kind_with_events_cannot_be_deleted(login, org):
    ...
    assert login("admin").delete(f"/api/competitions/kinds/{training_id}").status_code == 400


# --- per-kind automatic roles ----------------------------------------------
def test_roles_fire_only_for_their_kind(login, org):
    root = ensure_position(login("admin"))
    _template(login, title="{event} PM", event="competition_created", kind_slug="competition")
    _template(login, title="{event} Season Lead", event="competition_created", kind_slug="training")
    _template(login, title="{event} Observer", event="competition_created", kind_slug=None)  # all kinds
    ...
    # competition got its PM + the shared Observer, but NOT the training role
...
def test_chain_stays_within_a_kind(login, org):
    ...
    competition role, even though both react to the same event triggers."""
    root = ensure_position(login("admin"))
    _template(login, title="{event} PM", event="competition_created", kind_slug="competition")
    _template(login, title="{event} Season Lead", event="competition_created", kind_slug="training")
    ...
    cat = login("admin").post(f"/api/competitions/{training['id']}/categories", json={"name": "Beginners"}).json()
    login("admin").post(f"/api/competitions/categories/{cat['id']}/teams", json={"name": "Alpha"})
