...
ordering, the single ask-once root, event-triggered seating (event/
team/member), multi-occupancy, seat-level authority (a seat whose access
level carries events.manage_seated makes its occupants that entity's
...
def _team(login, cat_id, root=None, who="cto", name="Alpha"):
    ...
    if root is not None:
        body["role_root_position_id"] = root
    r = login(who).post(f"/api/events/categories/{cat_id}/teams", json=body)
    assert r.status_code == 201, r.text
    return r.json()


def test_template_crud_auto_orders_and_renumbers(login, org):
    a = _template(login, title="{event} PM", event="event_created")
    ...
    assert login("cto").post(
        ...
        json={"title_template": "x", "event": "event_created"},
    ).status_code == 403


def test_first_role_position_requires_a_parent_then_remembers_it(login, org):
    _template(login, title="{event} PM", event="event_created")
    r = login("cto").post("/api/events", json={"name": "No Root Yet"})
    ...
    # a second event doesn't need to ask again
...
def test_seats_start_vacant_and_seat_level_confers_scoped_management(login, org):
    _template(login, title="{event} PM", event="event_created", level="Lead")
    ...
    assert login("cto").get(f"/api/events/{comp['id']}").json()["can_manage"] is False

    # appoint the creator + a plain staff member — the seat holds both, and
    # the SEAT's level (Lead, which carries events.manage_seated) makes
    ...
    assert login("cto").get(f"/api/events/{comp['id']}").json()["can_manage"] is True
    assert login("sw_emp").get(f"/api/events/{comp['id']}").json()["can_manage"] is True
    # someone not seated still can't manage it
    assert login("student").get(f"/api/events/{comp['id']}").json()["can_manage"] is False
    ...
    assert login("sw_emp").get(f"/api/events/{comp['id']}").json()["can_manage"] is False
...
def test_four_level_chain_seats_correctly(login, org):
    """pm (event) -> coach (team) -> team_lead (team) -> member (membership)."""
    _template(login, title="{event} PM", event="event_created", level="Lead")
...
def test_team_lead_role_grants_member_management(login, org):
    ...
    detail = login("student").get(f"/api/events/{comp['id']}").json()
...
def test_retitle_cascades_through_the_whole_chain(login, org):
    ...
    login("cto").patch(f"/api/events/{comp['id']}", json={"name": "New Comp"})
    login("cto").patch(f"/api/events/teams/{team['id']}", json={"name": "New Team"})
...
def test_archive_removes_the_whole_subtree_and_reactivate_rebuilds_it(login, org):
    ...
    stud = org["student"].full_name
    login("cto").patch(f"/api/events/{comp['id']}", json={"status": "archived"})
    tree = _tree_by_title(login)
    ...
    # sits there
    login("admin").patch(f"/api/events/{comp['id']}", json={"status": "active"})
    tree = _tree_by_title(login)
    ...
    assert login("cto").delete(f"/api/events/{comp['id']}").status_code == 403

    assert login("admin").delete(f"/api/events/{comp['id']}").status_code == 204
    tree = _tree_by_title(login)
    assert "Doomed PM" not in tree and "Squad Lead" not in tree and stud not in tree
...
def test_team_soft_and_permanent_delete_both_remove_positions(login, org):
    ...
    login("cto").post(f"/api/events/teams/{team_a['id']}/members", json={"user_id": org["student"].id})
    assert login("cto").delete(f"/api/events/teams/{team_a['id']}").status_code == 204
    tree = _tree_by_title(login)
    assert "Soft Lead" not in tree and stud not in tree

    team_b = _team(login, cat["id"], name="Gone")
    login("cto").post(f"/api/events/teams/{team_b['id']}/members", json={"user_id": org["comp_member"].id})
    assert login("admin").delete(f"/api/events/teams/{team_b['id']}?permanent=true").status_code == 204
    tree = _tree_by_title(login)
...
def test_reordering_a_template_live_reparents_existing_positions(login, org):
    ...
    cat = login("admin").post(f"/api/events/{comp['id']}/categories", json={"name": "Senior"}).json()
...
def test_template_level_edit_propagates_to_existing_positions(login, org):
    ...
    tpl = _template(login, title="{event} PM", event="event_created")
    comp = _comp(login, root=ensure_position(login("admin")))
    seat_role(login("admin"), comp, [org["cto"].id])
...
def test_parent_template_id_previews_the_chain_before_anything_real_exists(login, org):
    ...
    before any event/team exists — parent_template_id (purely
    structural, no positions needed yet) is what makes that possible."""
    pm = _template(login, title="{event} PM", event="event_created")
    coach = _template(login, title="{team} Coach", event="team_created")
    lead = _template(login, title="{team} Lead", event="team_created")
    deputy = _template(login, title="{event} Deputy PM", event="event_created")
    ...
    # for a event-level role — still resolves to pm, not coach
...
def test_insert_after_nests_new_role_under_the_clicked_parent(login, org):
    ...
    pm = _template(login, title="{event} PM", event="event_created")
    deputy = _template(login, title="{event} Deputy", event="event_created")
    ...
    # event-level role, so the insertion doesn't steal deputy away
    assert templates[deputy["id"]]["parent_template_id"] == pm["id"]
...
def test_template_added_after_entity_exists_blocks_dependents_instead_of_orphaning_to_root(login, org):
    ...
    # cto has no role seat on this old event (PM never applied to it) —
    # only manage_any levels can manage it until someone configures things
    old_cat = login("admin").post(f"/api/events/{old_comp['id']}/categories", json={"name": "Senior"}).json()
    old_team = _team(login, old_cat["id"], who="admin", name="Alpha")  # no root passed — must never be needed

    tree = _tree_by_title(login)
    assert "Preexisting PM" not in tree  # never backfilled onto the pre-existing event
    assert "Alpha Coach" not in tree  # blocked: its PM prerequisite doesn't exist for this event
    assert old_team["roles"][0]["position_id"] is None
    assert old_team["roles"][0]["occupants"] == []

    # a *new* event, created after both templates exist, chains normally
    root = ensure_position(login("admin"))
    new_comp = _comp(login, root=root, who="cto", name="Fresh")
    seat_role(login("admin"), new_comp, [org["cto"].id])
    new_cat = login("cto").post(f"/api/events/{new_comp['id']}/categories", json={"name": "Senior"}).json()
